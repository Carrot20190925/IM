...
static NSString *const kURLSchemePigramKey                = @"pigram";
...
- (void)getWakeUpParams:(OpeninstallData *)appData{
    if (appData.data) {//(动态唤醒参数)
        ...
        NSString *short_id = appData.data[@"short_id"];
        if (short_id.length > 0) {
            ...
            [self tapLinkActionWithLink:short_id];
            self.link = nil;
            return;
        }
...
- (BOOL)application:(UIApplication *)app openURL:(NSURL *)url options:(NSDictionary<UIApplicationOpenURLOptionsKey,id> *)options{
    ...
    if ([url.scheme isEqualToString:kURLSchemePigramKey]&&[url.host isEqualToString:@"p.land"]){
        if (url.lastPathComponent.length > 0) {
            self.link = url.lastPathComponent;
        }
    }
    ...
    if ([url.scheme isEqualToString:kURLSchemePigramKey]&&[url.host isEqualToString:@"p.land"]){
        if (url.lastPathComponent.length > 0) {
            [self tapLinkActionWithLink:url.lastPathComponent];
        }
        self.link = nil;
        return  YES;
    }
...
//        OnboardingController
    ...
    }else
    {
        ...
        if (self.link.length > 0) {
            [self tapLinkActionWithLink:self.link];
            self.link = nil;
        }
    }
