                                ...
                                block:^(void (^benchmarkCompletion)(void)) {
                                    [self.databaseStorage asyncWriteWithBlock:^(SDSAnyWriteTransaction *writeTransaction) {
                                        ...
                                        if (mediaAttachments.count > 0) {
                                            [SSKEnvironment.shared.messageSender sendMessage:outgoingMessagePreparer success:^{
...
+ (TSOutgoingMessage *)pg_enqueueMessageWithContactShare:(PGMessageShare *)contactShare inThread:(TSThread *)thread transaction:(SDSAnyWriteTransaction *)transaction
{
    OWSAssertIsOnMainThread();
    OWSAssertDebug(contactShare);
    OWSAssertDebug(thread);
