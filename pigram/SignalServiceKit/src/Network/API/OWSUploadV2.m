...
@interface OWSAvatarUploadV2 ()<OWSAttachmentUploadV2Protocol>
                     ...
                     progressBlock:(UploadProgressBlock)progressBlock
{
    ...
    AnyPromise *promise = [AnyPromise promiseWithResolverBlock:^(PMKResolver resolve) {
        ...
        [strongSelf uploadFile:strongSelf.avatarData name:form.formKey resovel:resolve progressBlock:progressBlock];
        /*
...
@interface OWSAttachmentUploadV2 () <OWSAttachmentUploadV2Protocol>
...
///上传群头像
- (AnyPromise *)uploadGroupAvatarToService:(NSData *)avatarData clearLocalAvatar:(dispatch_block_t)clearLocalAvatar progressBlock:(UploadProgressBlock)progressBlock group_id:(nonnull NSString *)group_id{

    self.data = avatarData;
    AnyPromise *promise = [AnyPromise promiseWithResolverBlock:^(PMKResolver resolve) {
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        
            [self uploadGroupAvatarAttachmentToService:resolve progressBlock:progressBlock skipWebsocket:NO group_id:group_id];
...
- (void)uploadGroupAvatarAttachmentToService:(PMKResolver)resolve
                    progressBlock:(UploadProgressBlock)progressBlock
                    skipWebsocket:(BOOL)skipWebsocket group_id:(nonnull NSString *)group_id{
    ...
    void (^formSuccess)(id _Nullable) = ^(id _Nullable formResponseObject) {
        ...
        [[strongSelf parseFormAndUpload:formResponseObject urlPath:@"" progressBlock:progressBlock]
                .thenInBackground(^{
                    OWSUploadForm *_Nullable form = [OWSUploadForm parse:formResponseObject];
                    TSRequest *request = [OWSRequestFactory groupAvatarRequest:form.formKey group_id:group_id];
                    [self.networkManager makeRequest:request
                        success:^(NSURLSessionDataTask *task, id _Nullable formResponseObject) {
                        //                 /
                            resolve(form.formKey);
                        }
                        failure:^(NSURLSessionDataTask *task, NSError *error) {
                            resolve(error);
                        }];
                    ...
                    skipWebsocket:(BOOL)skipWebsocket
{
    ...
    void (^formFailure)(NSError *) = ^(NSError *error) {
        OWSLogError(@"Failed to get profile avatar upload form: %@", error);
//        NSString *tag = OWSLogPrefix();
        NSString  * fail = [[NSString alloc]initWithFormat:@"获取图片id失败%@", error.code];
        resolve(error);
    };
                     ...
                     progressBlock:(UploadProgressBlock)progressBlock
{
    ...
    __weak OWSAttachmentUploadV2 *weakSelf = self;
    AnyPromise *promise = [AnyPromise promiseWithResolverBlock:^(PMKResolver resolve) {
        NSData *_Nullable uploadData =  weakSelf.data;
        
        
        if (uploadData.length < 1) {
            uploadData = [weakSelf attachmentData_none_encrypt];
        ...
        [self uploadFile:uploadData name:form.formKey resovel:resolve progressBlock:progressBlock];
